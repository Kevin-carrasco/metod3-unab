---
title: "Práctico 08. Regresión logística II"
...
```
  ...
  * Como la universidad no permite el acceso a Github, pueden encontrar la base de datos en el canvas del curso, archivos, bajo el nombre datos-practico8.RData

  * Si no están conectados mediante eduroam de la universidad, pueden usar el siguiente link:
...
1- Estima un modelo de regresión logística que tenga como variable dependiente `petition_dummy` (Frecuence de participación: firmar una petición). Propon la mejor forma de estimar la participación en peticiones a partir de las siguientes variables: `Unionized` (afiliación sindicatos), `pol_pos_left` (dummy ser de izquierda o no), `civic_involvement` (compromiso cívico), `X003` (edad), `female` (mujer), `educ` (educación).

Antes de estimar los modelos, reflexionar entorno a:

  - ¿Qué variable sería el principal predictor de firmar una petición?
  - ¿Se deben agregar primero las variables socioeconómicas o las de opinión pública?
  - Van todas las variables independientes juntas, se agregan unas primero y otras después?

2- Interpretar los coeficientes de los modelos estimados.

3- Estimar y graficar las probabilidades predichas de firmar una petición para las variables independientes `pol_pos_left` y `civic_involvement`