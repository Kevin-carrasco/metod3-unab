...
1. Estimar los coeficientes $b_0$ y $b_1$ **a mano**, replicando el procedimiento visto en clases
2. Estimar el mismo modelo con la función `lm()` y comprobar que los resultados coinciden
3. **Interpretar** el intercepto y la pendiente en términos sustantivos
4. Obtener **valores predichos** y **residuos**, y utilizarlos para predecir
5. Comprender por qué el criterio de estimación se denomina **mínimos cuadrados ordinarios**
...
Una solución habitual consiste en **centrar** la variable independiente, es decir, restarle su promedio. Así, el valor 0 pasa a representar a una persona de edad promedio:

```{r}
proc_data$edad_c <- proc_data$edad - mean(proc_data$edad)

modelo_c <- lm(confianza_vecinos ~ edad_c, data = proc_data)
coef(modelo_c)
```

Observe que la **pendiente no cambia**: centrar desplaza el origen del eje X, pero no altera la inclinación de la recta. Lo que cambia es el intercepto, que ahora corresponde a la confianza esperada de una persona con edad promedio.
